...
class OrderDataStore:
    """
    Handles product catalog, discount calculation, order totals, and persistence.
    All deterministic operations are seeded for reproducible grading.
    """
    ...
    def __init__(self, data_dir: Path, output_dir: Path, *, today: str | None = None) -> None:
        ...
        # Vietnamese / alias category mapping
    ...
    def _canonicalize_category(self, value: str | None) -> str | None:
        if not value:
            return None
    ...
    def build_detail_token(product_ids: list[str]) -> str:
        """Deterministic token based on sorted product IDs."""
    ...
    def list_products(
        ...
        *,
        ...
        Returns compact catalog summaries for the model to use in later tool calls.
        """
        ...
        wanted_category = self._canonicalize_category(category)
        ...
        for product in self.products:
            ...
            # Skip products that match no query terms at all
            ...
            results.append(
                (
                    ...
                    product.product_id,
                    {
                        ...
                        "next_step": (
                            "Call get_product_details with the chosen product_id list "
                            "to verify price, stock, and obtain the detail_token."
                        ),
                    },
                )
            )
    ...
    def get_product_details(self, product_ids: list[str]) -> dict:
        """
        Return exact pricing, stock, and warranty for each product ID.
        Generates a deterministic validation token for downstream tools.
        """
        details: list[dict] = []
        ...
        found_ids = [item["product_id"] for item in details if item.get("status") == "ok"]
        return {
    ...
    def get_discount(self, *, seed_hint: str, customer_tier: str = "standard") -> dict:
        """
        Simulate a deterministic campaign discount (0.1 or 0.2) seeded by customer identifier.
        """
    ...
    def calculate_order_totals(
        ...
        *,
        ...
        """
        Validate stock, verify detail_token, compute subtotal, discount, and final total.
        Returns an error payload for common user mistakes instead of raising.
        """
        if discount_rate not in {0.1, 0.2}:
            return {"status": "error", "errors": [f"Unsupported discount_rate: {discount_rate}. Must be 0.1 or 0.2."]}

        requested_ids = [item.product_id for item in items]
        if not self.validate_detail_token(requested_ids, detail_token):
            return {
                "status": "error",
                "errors": ["Invalid detail_token. Call get_product_details again for this product set before pricing."],
            }
    ...
    def save_order(
        ...
        *,
        ...
        """
        Recompute totals before saving, build deterministic order ID, persist JSON to output dir.
        Returns both the saved payload and the absolute file path.
        """
        ...
        # Build a deterministic order ID from customer identity + items
